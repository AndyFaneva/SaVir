...
<div style="margin-top:10%">
    <?php
        ...
        $req->execute(Array($nomuniv));
        ?>
        <?php
      while($user=$req->fetch()) {
        ?>
        <div class="standuniv" style="backgroun-color:grey" >
            <div class="stand" style="width:50%;margin-left:auto;margin-right:auto;box-shadow:15px 15px 15px 15px black;margin-top:3%">        
         <?php
         echo "<h1>".$user['mention']."<h1>";
         echo "<h1>".$user['descrimention']."<h1>";
         ?>
         <img   src="image2.php?id=<?php echo $user['id'];?>"  width="350"></td>
         <form action="conférence.php" method="post">
         <input type="hidden" name="action" value="<?php echo $user['id'] ?>">
    ...
    border-radius: 4px;
    cursor: pointer;
    width: 100%;">Ajouter conférence</button>
         </form>
         <form action="ajouterstand.php" method="post">
<input type="hidden" name="action" value="<?php echo $user['id'] ?>">
<button type='submit' style="  background-color: rgb(46, 82, 245);
    color: white;
    padding: 14px 20px;
    ...
    border-radius: 4px;
    cursor: pointer;
    width: 100%;">Ajouter stand</button>
</form> 
<form action="supprimerstand.php" method="post">
<input type="hidden" name="action" value="<?php echo $user['id'] ?>">
